...
export default function ProjectsCard({
  ...
  const { status } = useMyMembershipStatus(filteredProject.id, loggedInUserId);

  return (
    <>
      <Card style={{ borderRadius: 5, width: 310, height: 230 }}>
        <CardMedia
          component="img"
          height="90"
          alt="projects"
        ...
        <CardActionArea onClick={() => handleOpen(filteredProject.id)}>
          <Box className={classes.box}>
            ...
            {isApplications ? (
              <Typography className={classes.isApplications}>
                {status === 'accepted' ? (
                  <DoneOutlinedIcon
                    fontSize="small"
                    style={{ color: 'green' }}
